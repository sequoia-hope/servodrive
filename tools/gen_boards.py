...
BOARDS = {
    ...
    "single_board": dict(
        ...
        note=("Board A plus the minimum of board B. 48 V max. XT30 bus input, TVS and "
              "cans, two LMR38010 rails, USB-C, RS-485 relay on two SH 6, expansion header."),
        # the root sheet's comments: board S's sheets are drawn, not a skeleton
        comments=("Sheets drawn by tools/schlayout_s.py from the netlist in tools/schematic_s.py",
                  "See ../../single.html"),
    ),
    "power_board": dict(
...
CAPTURE = ("Sheet skeleton generated by tools/gen_boards.py",
           "Capture is manual from here. See ../../spec.html")
...
def sch_notes(cfg, sheets):
    ...
    o = ['(kicad_sch', '\t(version 20250114)', '\t(generator "eeschema")',
         ...
         f'\t\t(company "Sequoia Hope Alexander")\n'
         f'\t\t(comment 1 "{cfg.get("comments", CAPTURE)[0]}")\n'
         f'\t\t(comment 2 "{cfg.get("comments", CAPTURE)[1]}")\n'
...
def emit_s(cfg, force, pcb_too=True):
    ...
    for nm, desc in SHEETS_S:
        ...
        # drawn by hand-written layouts, and checked against the netlist
        # before it is written (tools/schlayout_s.py)
        text = SCHL.sheet_text(nm, cs, cfg["name"], uid(cfg["name"], "subsch", nm),
                               uid(cfg["name"], "sheet", nm), f"{cfg['title']} — {nm}",
                               flags=SCHS.SHEET_FLAGS.get(nm, ()), glob=glob,
                               root_uuid=uid(cfg["name"], "sch"))
